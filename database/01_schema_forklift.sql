...
CREATE TABLE bom_version (
    ...
    CONSTRAINT uk_bom_version_owner UNIQUE (version_id, material_id),
...
ALTER TABLE material ADD CONSTRAINT fk_material_cur_ver
    FOREIGN KEY (current_version_id, material_id)
    REFERENCES bom_version(version_id, material_id);
...
CREATE TABLE bom (
    ...
    CONSTRAINT fk_bom_version_owner FOREIGN KEY (version_id, parent_material_id)
        REFERENCES bom_version(version_id, material_id),
...
CREATE TABLE production_order (
    ...
    CONSTRAINT fk_prod_order_version_owner FOREIGN KEY (version_id, material_id)
        REFERENCES bom_version(version_id, material_id),
...
CREATE TABLE external_order (
    ...
    status          VARCHAR2(20) NOT NULL
                    ...
                    CHECK (status IN ('待审核','已接受','已拒绝','已转换','已交货')),
...
CREATE TABLE external_order_delivery (
    delivery_id    NUMBER(10) GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    ext_order_id   NUMBER(10) NOT NULL REFERENCES external_order(ext_order_id),
    material_id    NUMBER(10) NOT NULL REFERENCES material(material_id), -- 交货时点产品快照
    quantity       NUMBER(12,2) NOT NULL CHECK (quantity > 0),            -- 当前仅支持整单交货
    delivery_time  TIMESTAMP NOT NULL,
    operator_id    NUMBER(10) NOT NULL REFERENCES sys_user(user_id),
    CONSTRAINT uk_ext_order_delivery_order UNIQUE (ext_order_id)
);
...
CREATE TABLE stock_alert (
    ...
    material_id    NUMBER(10) NOT NULL REFERENCES material(material_id),
    alert_type     VARCHAR2(20 CHAR) DEFAULT '低库存' NOT NULL,
...
CREATE TABLE batch_consumption (
    ...
    CONSTRAINT uq_bc_order_item UNIQUE (order_id, item_id)
);

-- 完工批次的原材料消耗增量快照。batch_consumption 保存订单累计量，
-- 本表将两次报工之间的新增消耗固定到具体成品批次，保证分批追溯准确。
CREATE TABLE finish_batch_consumption (
    inbound_id       NUMBER(10) NOT NULL REFERENCES finish_inbound(inbound_id),
    item_id          NUMBER(10) NOT NULL REFERENCES purchase_order_item(item_id),
    consume_qty      NUMBER(12,2) NOT NULL CHECK (consume_qty > 0),
    allocated_time   TIMESTAMP DEFAULT SYS_EXTRACT_UTC(SYSTIMESTAMP) NOT NULL,
    CONSTRAINT pk_finish_batch_consumption PRIMARY KEY (inbound_id, item_id)
);
...
CREATE INDEX idx_fbc_item            ON finish_batch_consumption(item_id, inbound_id);
CREATE UNIQUE INDEX uq_active_stock_lock ON stock_lock (
    CASE WHEN status = '已锁定' THEN order_id END,
    CASE WHEN status = '已锁定' THEN material_id END
);
...
-- PO5 宁德时代 · 锂电池首批 · 已完成
INSERT INTO purchase_order VALUES (5,5,'已完成','2026-05-12','2026-05-30','2026-05-28',2,825000.00);
INSERT INTO purchase_order_item VALUES ( 9,5,402,30,30,27500.00);
INSERT INTO receive_record VALUES (10,5,402,30,'2026-05-28');
-- PO6 正新橡胶 · 已完成
...
INSERT INTO finish_inbound VALUES (6,6,101, 2,  8,  8,'CPD30-20260706-B01','2026-07-06 17:30:00',5);
...
-- 将订单累计消耗快照到实际完工批次。订单 6 为部分完工批次，后续新增消耗
-- 将由领域过程仅按增量分配到新的 finish_inbound。
INSERT INTO finish_batch_consumption VALUES (1, 8,  102,'2026-05-30 16:40:00');
INSERT INTO finish_batch_consumption VALUES (3, 7,   40,'2026-06-06 17:10:00');
INSERT INTO finish_batch_consumption VALUES (4, 4,  639,'2026-06-14 16:55:00');
INSERT INTO finish_batch_consumption VALUES (4,15,  205,'2026-06-14 16:55:00');
INSERT INTO finish_batch_consumption VALUES (4, 5,  100,'2026-06-14 16:55:00');
INSERT INTO finish_batch_consumption VALUES (5, 9,   15,'2026-06-28 18:20:00');
INSERT INTO finish_batch_consumption VALUES (5,10,   30,'2026-06-28 18:20:00');
INSERT INTO finish_batch_consumption VALUES (5,11,   30,'2026-06-28 18:20:00');
INSERT INTO finish_batch_consumption VALUES (5, 5,   30,'2026-06-28 18:20:00');
INSERT INTO finish_batch_consumption VALUES (5, 1, 6848,'2026-06-28 18:20:00');
INSERT INTO finish_batch_consumption VALUES (5, 3,13564,'2026-06-28 18:20:00');
INSERT INTO finish_batch_consumption VALUES (6, 5,   48,'2026-07-06 17:30:00');
INSERT INTO finish_batch_consumption VALUES (6, 9,    8,'2026-07-06 17:30:00');
INSERT INTO finish_batch_consumption VALUES (6, 3,10639,'2026-07-06 17:30:00');
...
ALTER TABLE external_order_delivery MODIFY (delivery_id GENERATED BY DEFAULT ON NULL AS IDENTITY (START WITH LIMIT VALUE));
