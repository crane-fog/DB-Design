/*
 * 数据库设计项目 API
 *
 * 项目 OpenAPI 总入口，按业务模块汇总接口定义。
 *
 * The version of the OpenAPI document: 0.1.0
 * 
 * Generated by: https://openapi-generator.tech
 */

using System;
using System.Linq;
using System.Text;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Org.OpenAPITools.Converters;

namespace Org.OpenAPITools.Models
{ 
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class ExternalOrderConvertResult : IEquatable<ExternalOrderConvertResult>
    {
        /// <summary>
        /// Gets or Sets ExtOrderId
        /// </summary>
        [Required]
        [DataMember(Name="ext_order_id", EmitDefaultValue=true)]
        public long ExtOrderId { get; set; }

        /// <summary>
        /// Gets or Sets ProductionOrders
        /// </summary>
        [Required]
        [DataMember(Name="production_orders", EmitDefaultValue=false)]
        public List<ProductionOrderBrief> ProductionOrders { get; set; }

        /// <summary>
        /// 外部订单与生产订单关联表 external_order_production 的写入结果。
        /// </summary>
        /// <value>外部订单与生产订单关联表 external_order_production 的写入结果。</value>
        [Required]
        [DataMember(Name="associations", EmitDefaultValue=false)]
        public List<ExternalOrderProductionAssociation> Associations { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class ExternalOrderConvertResult {\n");
            sb.Append("  ExtOrderId: ").Append(ExtOrderId).Append("\n");
            sb.Append("  ProductionOrders: ").Append(ProductionOrders).Append("\n");
            sb.Append("  Associations: ").Append(Associations).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (obj is null) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((ExternalOrderConvertResult)obj);
        }

        /// <summary>
        /// Returns true if ExternalOrderConvertResult instances are equal
        /// </summary>
        /// <param name="other">Instance of ExternalOrderConvertResult to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(ExternalOrderConvertResult other)
        {
            if (other is null) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    ExtOrderId == other.ExtOrderId ||
                    
                    ExtOrderId.Equals(other.ExtOrderId)
                ) && 
                (
                    ProductionOrders == other.ProductionOrders ||
                    ProductionOrders != null &&
                    other.ProductionOrders != null &&
                    ProductionOrders.SequenceEqual(other.ProductionOrders)
                ) && 
                (
                    Associations == other.Associations ||
                    Associations != null &&
                    other.Associations != null &&
                    Associations.SequenceEqual(other.Associations)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    
                    hashCode = hashCode * 59 + ExtOrderId.GetHashCode();
                    if (ProductionOrders != null)
                    hashCode = hashCode * 59 + ProductionOrders.GetHashCode();
                    if (Associations != null)
                    hashCode = hashCode * 59 + Associations.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(ExternalOrderConvertResult left, ExternalOrderConvertResult right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(ExternalOrderConvertResult left, ExternalOrderConvertResult right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
